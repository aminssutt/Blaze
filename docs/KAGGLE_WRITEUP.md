...
- **Implementation status**: **Implemented** (`agents/radio_intelligence/`, PR #84, 23 passing tests with mocked Gemma responses). French system prompt with firefighter lexicon (CCF, PC, VL, D17…) and explicit STT-noise normalization instructions. Deterministic post-LLM guardrails: evidence spans must fuzzy-match the transcript (invented evidence caps confidence at 0.3 + uncertainty flag), unknown units forced to `"unknown"`, `source_type` forced to `human_report`, corrections without a target resolved deterministically against recent context, final validation against the frozen `radio_event` schema.
- **Current limitations**: live Gemma 4 validation pending GPU access — all tests currently run against canned model responses; extraction quality on real inference not yet measured.
...
- **Implementation status**: **Implemented** (`agents/situation_context/`, PR #86, 7 passing tests). Two-phase design: guarded tool selection (hallucinated tool names discarded without execution, per-turn call budget of 5) then snapshot synthesis with a **provenance lock**: source labels are rewritten from the real `ToolResult`s, so the model cannot claim cached data is live (dedicated test). Tests run against the real committed cache data.
- **Current limitations**: live tool-selection quality unmeasured (canned responses); default catalog names differ from the executor registry names — bridged via `catalog_from_registry()` at integration time.
...
- **Implementation status**: **Implemented** (`agents/tactical_planning/`, PR #85, 8 passing tests). Evidence IDs are verified against real event/tool-call IDs (invented references removed + flagged); plan version, ID and timestamps generated by code, never by the model; append-only `PlanHistory` keeps every version immutable; `human_approval_required` forced `true` for high/critical actions; bounded tool loop (2 rounds) for `compute_route`.
- **Current limitations**: plan quality on live inference unmeasured; multi-incident support out of scope.
...
- **Implementation status**: **Implemented** (`agents/dispatch/`, PR #88, 9 passing tests). Hard authorization precondition: a `reject` decision (or mismatched plan ID) raises **before any LLM call is made** (asserted: zero HTTP requests). Anti-invention guardrail: a closed vocabulary of scenario locations (from the seeded roads/resources data) is checked lexically — a route or destination absent from the approved action triggers bounded regeneration then a typed error. Acknowledgement forced for high/critical priorities; TTS-friendly phrasing rules ("CCF" → "camion-citerne").
- **Current limitations**: message naturalness on live inference unmeasured; lexical guard covers locations/numbers, not full semantics.
...
- **How JSON extraction works**: all five agents call a shared client (`agents/common/inference_client.py`, PR #78) through `chat_structured()`: the request carries a `json_schema` response format; the response is validated with `jsonschema` against the frozen contract; on failure, a bounded **repair loop** (default 2 attempts) re-prompts with the exact validation error, then raises a typed `StructuredOutputError`. Retries with exponential backoff cover timeouts and 5xx (never 4xx). Live failure rates: TODO (pending GPU).
- **How tools are selected**: agents receive a tool catalog and propose `ToolRequest`s with a stated reason; the Tool Execution Layer (`backend/orchestrator/`, PR #79) validates arguments against per-tool JSON Schemas and an allowlist — an unknown or unavailable tool is **never executed**, and every request/result pair is audited against the frozen contracts. Observed selection quality on live inference: TODO.
- **How hallucinations are reduced**: layered deterministic guardrails around every LLM output — (1) evidence spans must fuzzy-match the source transcript or confidence is capped; (2) provenance labels are rewritten from real tool results, so the model cannot claim invented sources; (3) plan evidence IDs are verified against real event/tool IDs; (4) dispatch messages are checked against a closed location vocabulary from the approved action; (5) the Safety Critic's mechanical rule failures override any LLM approval; (6) the evaluation runner counts unsupported facts. Measured live results: TODO.
- **How uncertainty is handled**: explicit `confidence`, `confirmation_status` (reported / inferred / confirmed), and `uncertainties` fields flow from extraction through planning to the human approval screen; guardrail interventions (degraded evidence, unknown units, removed references) are themselves appended as uncertainties rather than silently dropped. Examples from real runs: TODO.
- **A cloud-call guard is built into the client**: any non-localhost inference URL raises `RemoteInferenceBlockedError` unless a dev-only escape flag is set, and a shared `CallLog` counts `cloud_calls` so the demo can assert it equals 0 live.
...
10. **Hard human approval gate in the state machine.** Approval is not a UI convention — the `AWAITING_HUMAN_APPROVAL` state makes dispatch structurally impossible before an explicit decision (the transition does not exist; verified by the state-machine test matrix, 41 tests, PR #73). The Dispatch Agent additionally refuses to issue a single LLM call without an `approve` decision (PR #88).
11. **Hybrid Safety Critic (rules + LLM) instead of LLM-only review.** We initially considered a pure LLM critic; we shipped a hybrid where 8 safety rules are deterministic code and the LLM can only add objections or escalate — never soften a mechanical failure. Reason: an LLM can be talked out of an objection; a water-threshold check cannot (PR #87).
12. **Guardrails as post-processing, not prompt hopes.** Every agent pairs its prompt with deterministic post-LLM validation (evidence matching, ID verification, closed-vocabulary checks, forced fields). Reason: prompts reduce error rates, code bounds them.
13. **External store + `useSyncExternalStore` for the frontend stream state** (no Redux/zustand dependency). Reason: replay timers live outside React; at x5 speed a Context-based approach would re-render the whole tree several times per second (PR #82). The mock player and the future real SSE source share one `EventSource` adapter interface, so integration swaps the source, not the consumers.
14. **Pinned `av==13.1.0` for faster-whisper audio decoding.** The newest PyAV wheel was blocked by Windows Smart App Control on the dev machine (unsigned recent binary); the older established wheel loads fine. Documented in the STT requirements (PR #69).
15. TODO — keep appending decisions made during the build ("we initially planned X, switched to Y because…").
...
1. **Windows Smart App Control blocked PyAV DLLs** (STT hello-world, PR #69). The latest `av` 18.x wheel — unsigned and too recent to have cloud reputation — was refused by the OS. Solved by pinning `av==13.1.0` (established wheel); documented in requirements so it doesn't resurface on other machines.
2. **Whisper `small` mangles domain proper nouns on radio audio**: real transcription rendered "D17" as "dédicite" and "hangar" as "Jean-Garre" (measured on our own recordings). Mitigations shipped: the Radio Intelligence prompt normalizes noisy mentions with reduced confidence and keeps the original in `uncertainties`; the STT service (ticket #12) is testing faster-whisper's `initial_prompt` with the firefighter lexicon — effect will be reported honestly, whether or not it helps.
3. **Duplicate work on ticket #21**: a human teammate and a Claude agent both implemented the FastAPI scaffold concurrently (PRs #67 and #68). The agent rebased its additions onto the human's merged conventions. Fixed with a team process: claim a ticket by commenting on the issue before starting.
4. **Speaker labeling error found by listening to the real audio**: the roadmap assumed Audio 4 was spoken by Charlie 1; the actual recording is Alpha 3. The manifest was corrected against the recordings rather than the spec (PR #64) — ground truth beats documentation.
5. **A real bug caught by tests before merge**: the Safety Critic's water-refill check initially counted a retreat to Water Point 2 as a refill plan; the test suite exposed it and it was fixed pre-merge (PR #87).
...
| Metric | Value |
|---|---|
| Number of autonomous agents | 5 — all implemented (Radio Intelligence, Situation Context, Tactical Planning, Safety Critic, Dispatch) |
