 ...
 *
 * This module is the ONLY place that knows the concrete source is
 * `MockStreamSource`. Ticket #54 swaps it for `SseSource` here; every
 * consumer (store, hooks, UI regions) is untouched.
 */
...
export function getSession(): Session {
  if (!session) {
    ...
    const player = new MockStreamSource();
